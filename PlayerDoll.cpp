...
APlayerDoll::APlayerDoll()
{
	...
	// 임시 팔 Sphere 위치 설정
	PlayerHandSocket->SetRelativeLocation(FVector(100.0f, 25.0f,-10.0f));
...
void APlayerDoll::BeginPlay()
{
	...
	// Pawn 초기 위치 설정
	FVector InitialPosition = FVector(0.0f, 0.0f, 100.0f);
...
void APlayerDoll::Tick(float DeltaTime)
{
	...
	if(TimeUnit > 1.0f)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("BPM : %f"), PlayerBPM);
		UE_LOG(LogTemp, Warning, TEXT("State : %d"), PlayerState);
		UE_LOG(LogTemp, Warning, TEXT("WalkCount : %d"), WalkButtonPressCount);
		UE_LOG(LogTemp, Warning, TEXT("RunCount : %d"), RunButtonPressCount);
	}
}
...
void APlayerDoll::MoveForward(float AxisValue)
{
	...
	NewLocation += this->GetActorForwardVector() * MoveSpeed; // 이동속도 결정
...
void APlayerDoll::MoveRight(float AxisValue)
{
	...
	NewLocation += this->GetActorRightVector() * MoveSpeed; 
	this->SetActorLocation(NewLocation);
}
